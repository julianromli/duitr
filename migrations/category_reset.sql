...
-- =============================================
-- Step 0: Error handling setup
-- =============================================

-- Set variables to track migration progress
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error setting up migration: %', SQLERRM;
END$$;

-- =============================================
-- Step 1: Reset the categories table
-- =============================================
...
-- Check and fix any 'other' category keys to use the proper prefixes
DO $$
BEGIN
    -- Check if 'other' exists in categories_backup and update it
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error updating other category: %', SQLERRM;
END$$;
...
-- =============================================
-- Step 2: Update the transactions table
-- =============================================
...
-- =============================================
-- Step 4: Clean up old column if it exists
-- =============================================
...
-- Add a database trigger to automatically handle new categories
...
-- Update the existing verify_category_id function if it exists
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_proc WHERE proname = 'verify_category_id'
    ) THEN
        -- Drop the function to recreate it
        DROP FUNCTION IF EXISTS verify_category_id CASCADE;
        
        -- Recreate it with the new column names
        ...
        -- Reapply triggers that might be using this function
        DROP TRIGGER IF EXISTS verify_transaction_category ON transactions;
...
-- Create a similar trigger for budgets
...
BEFORE INSERT OR UPDATE ON budgets
FOR EACH ROW
EXECUTE FUNCTION set_default_category_for_budget();
...
BEGIN
    ...
    -- Mark completion in progress table
    UPDATE migration_progress SET completed = TRUE WHERE step = 'functions_recreated';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'Error in final validation: %', SQLERRM;
        PERFORM log_migration_error('validation', SQLERRM);
END$$;
